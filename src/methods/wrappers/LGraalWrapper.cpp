...
void LGraalWrapper::generateGDVFile(int graphNum) {
    ...
    FileIO::deleteFile(rawGDVFileName);
    FileIO::deleteFile(rawGDVFileName+".gr_freq");
    for (int i = 0; i < 10; i++) {
        FileIO::deleteFile(rawGDVFileName+".cl_0"+to_string(i)+"_freq");
    }
    for (int i = 10; i < 73; i++) {
        FileIO::deleteFile(rawGDVFileName+".cl_"+to_string(i)+"_freq");
    }
}
...
void LGraalWrapper::deleteAuxFiles() {
    FileIO::deleteFile(g1NetworkFile);
    FileIO::deleteFile(g2NetworkFile);
    FileIO::deleteFile(lgraalOutputFile);
}

Alignment LGraalWrapper::run() {
    if (not FileIO::fileExists(similarityFile)) {
        if (alpha > 0) {
    ...
    if (not FileIO::fileExists(g1GDVFile)) {
        generateGDVFile(1);
    }
    if (not FileIO::fileExists(g2GDVFile)) {
        generateGDVFile(2);
    }
    generateAlignment();
